...
def test_one_subrow_per_hint():
    hinted = [(3, 12, 16, "a"), (5, 24, 28, "b"), (7, 33, 36, "c")]
    seq, meta, _ = _build(hinted, [_hint(900), _hint(901), _hint(902)])
    assert meta[0] == 3
...
def test_a_subrow_carries_only_its_own_hint():
    ...
    hinted = [(3, 12, 16, "a"), (5, 24, 28, "b")]
    seq, meta, _ = _build(hinted, [_hint(900), _hint(901)])
...
def test_history_before_the_hint_is_untouched():
    hinted = [(5, 24, 28, "b")]
    seq, meta, _ = _build(hinted, [_hint(901)])
...
def test_student_keeps_the_union_of_spans():
    ...
    hinted = [(3, 12, 16, "a"), (5, 24, 28, "b")]
    _, meta, _ = _build(hinted, [_hint(900), _hint(901)])
...
def test_first_turn_hint_joins_the_prefix():
    ...
    hinted = [(1, 0, 4, "a")]
    seq, meta, fallbacks = build_spliced_teacher_row(
...
def test_fallback_counted_when_no_header_precedes_the_span():
    hinted = [(3, 13, 17, "a")]  # 13-2=11,12 are not the header tokens
    _, meta, fallbacks = _build(hinted, [_hint(900)])
    assert fallbacks == 1
    assert meta[0] == 1
