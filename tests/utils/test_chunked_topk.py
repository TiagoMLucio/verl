...
@pytest.mark.parametrize("num_positions,chunk_size", [(7, 512), (16, 4), (33, 8), (512, 512)])
def test_forward_matches_reference(num_positions, chunk_size):
    torch.manual_seed(0)
...
@pytest.mark.parametrize("chunk_size", [4, 512])
def test_gradcheck(chunk_size):
    torch.manual_seed(2)
...
def test_peak_memory_is_bounded_by_chunk_size():
    ...
    if not torch.cuda.is_available():
        pytest.skip("needs a device to measure allocation")
